...
export function GameContainer({ children, className = '' }: GameContainerProps) {
    return (
        <div className="w-full h-screen bg-slate-900 flex items-center justify-center p-4 overflow-hidden relative">
            ...
            <motion.div
                className={`relative w-full max-w-6xl aspect-[4/3] md:aspect-[16/9] bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/10 overflow-hidden ${className}`}
